...
const index = () => {
  ...
  useEffect(() => {
    const authStateListener = onAuthStateChanged(auth, async (user) => {
      ...
      await getPostsByUserId(user?.uid).then((result) => {
  ...
  return (
    <>
      <Head>
        <title>{user?.mycompany} 팀페이지 - JOBCOC</title>
        ...
        <meta property="og:type" content={`${user?.mycompany} 팀페이지, JOBCOC`} />
        <meta property="og:title" content={`${user?.mycompany} 팀페이지, JOBCOC`} />
