      ...
      <div>
        ...
        <div className="mt-[2rem] w-[95%] mx-auto bg-white shadow rounded-lg border border-gray-200 p-4">
          <header className="px-4 py-4 border-b border-gray-100">
            <div className="font-semibold text-lg text-gray-800">내 관심기업</div>
          </header>
              ...
              <div className="text-sm flex flex-col flex-wrap">
                {user?.favorites?.length !== 0 ?
                  user?.favorites?.map((v) => (
                    <div key={v?.boardId} className="w-full flex flex-row items-center rounded-lg bg-sky-500 shadow mb-4 mr-4 py-2 px-2 justify-between">
                  ...
                  : <Empty title="아직 관심 기업보드가 없습니다." />
                }
                {!user?.favorites && <Empty title="아직 관심 기업보드가 없습니다." />}
          ...
          <div className="flex justify-end font-bold space-x-2 text-lg border-t border-gray-100 px-5 py-4">
            <div>총</div>
            <div className="text-blue-600">{user?.favorites?.length || 0}<span x-text="total.toFixed(2)">건</span></div>
          </div>
