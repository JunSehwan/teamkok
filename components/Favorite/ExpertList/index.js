...
const index = () => {
  ...
  return (
    <>
      <div>
        {/* <!-- Table --> */}
        <div className="mt-[2rem] w-[95%] mx-auto bg-white shadow rounded-lg border border-gray-200 p-4">
          <header className="px-4 py-4 border-b border-gray-100">
            <div className="font-semibold text-lg text-gray-800">현업전문가로 활동하는 기업</div>
          </header>
            ...
            <div className="w-full">
              ...
              <div className="text-sm flex flex-col flex-wrap">
                {user?.experts?.length !== 0 ?
                  user?.experts?.map((v) => (
                    <div key={v?.boardId} className="w-full flex flex-row items-center rounded-lg bg-green-500 shadow mb-4 mr-4 py-2 px-2 justify-between">
                  ...
                  : <Empty title="아직 현업전문가로 활동하는 기업보드가 없습니다." />
                }
                {!user?.experts && <Empty title="아직 현업전문가로 활동하는 기업보드가 없습니다." />}
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end font-bold space-x-2 text-lg border-t border-gray-100 px-5 py-4">
            <div>총</div>
            <div className="text-blue-600">{user?.experts?.length || 0}<span x-text="total.toFixed(2)">건</span></div>
          </div>
