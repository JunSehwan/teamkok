...
import { uploadPicturePreview, saveCompanyPicturePreviewChanges } from 'firebaseConfig';
import { setChangePictureOpen } from 'slices/board';
import { setBoardPicture, setBoardPicturePreview, setBoardPicturePreviewURL } from 'slices/board';
...
const ChangePicture = () => {
  ...
  function closeWindow() {
    dispatch(setChangePictureOpen(false));
  }
  ...
  async function addPicture(e) {
    ...
    const pictureImage = e.target.files;
    const picturePreviewURL = await uploadPicturePreview(pictureImage, singleSection?.id);
    closeWindow();
    dispatch(setBoardPicturePreview(picturePreviewURL));
  }
