      ...
      <div className='w-full rounded-lg lg:rounded-l-none'>
        ...
        <ul className="flex flex-col">
          <li className="border-gray-400 flex flex-row mb-2">
            <button onClick={onViewEdu} className="w-max transition duration-500 shadow ease-in-out transform hover:-translate-y-1 hover:shadow-lg select-none cursor-pointer bg-white dark:bg-gray-800 rounded-md flex flex-1 items-center p-4">
        ...
        {viewEdu ?
          <div className='transition-all ease-in duration-500'>
            {myEducations?.map((v, index) => (
              <a key={index} className="text-left hover:shadow-xl w-[100%] border-2 border-b-4 border-gray-200 rounded-xl hover:bg-gray-50">
                {/* <!-- Badge --> */}
                ...
                <div className="grid grid-cols-6 p-5 gap-y-2">
                  ...
                  <div className="col-span-5 md:col-span-4 ml-4">
                    ...
                    <p className="text-gray-400 text-[0.78rem]">{`${v?.start?.year}년 ${v?.start?.month}월 ~ ${v?.end?.year}년 ${v?.end?.month}`}</p>
                  </div>
