...
import { LogoImage } from 'components/Common/Navbar/LogoButton';

const AuthModal = ({ setShow, tabIndex, setTabIndex }) => {
  const modalEl = createRef();
  const handleClickOutside = (event) => {
    if (!modalEl?.current?.contains(event.target)) setShow(false);
  };
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            ...
            <div className="relative p-[1rem] flex-auto">
              ...
              >
                ...
                <h1
                  role="main"
                  className="text-[1.2rem] ml-[0.6rem] dark:text-white font-semibold leading-7 lg:leading-9 text-center text-gray-800"
                >
                  {tabIndex === 1 ? 'TEAMKOK 회원가입' : 'TEAMKOK 로그인'}
                </h1>
              </div>
              <div className="mt-6">
                <AuthTab
                  ...
                  handleCancelModal={() => setShow(false)}
                />
              </div>
            </div>
          </div>
