...
const Singo = ({ conversation }) => {
  ...
  }, [category, conversation?.id, conversation?.users, dispatch, etc, user?.userID, user?.username])
  ...
  return (
    <>
      <button
        className='px-2 text-sm w-fit text-red-600 z-1 whitespace-nowrap'
        onClick={openSingo}>
        신고
      </button>
            ...
            >
              <div className="max-w-[720px] py-4">
                ...
                >
                  ...
                  <option value={7}>폭력적 언어</option>
                  <option value={5}>혐오발언</option>
                  <option value={4}>거짓정보</option>
                  <option value={2}>기타 부적절한 언어</option>
