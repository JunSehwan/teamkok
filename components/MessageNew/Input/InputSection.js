...
import { EMOJI_REPLACEMENT, formatFileName } from "hooks/constants";
...
const InputSection = ({
  ...
  return (
    <>
      ...
      <div
        className={`py-2 w-full bg-white shadow-[3px_-1px_29px_-15px_rgba(0,0,0,0.56)] border-slate-300 flex h-14 items-stretch gap-1 border-t px-2 ${disabled ? "pointer-events-none select-none" : ""
          }`}
      >
        <button
          onClick={() => imageInputRef.current?.click()}
          className="text-primary flex flex-shrink-0 items-center text-xl px-2 hover:bg-slate-200 rounded-xl py-2"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
