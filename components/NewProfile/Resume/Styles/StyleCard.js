...
const StyleCard = ({ title, onChangeCategory, sub, src, tag, category, index, number }) => {


  return (
    <li key={index} className="mx-[auto] border-gray-400 flex flex-row mb-2 w-[100%]">
      <button onClick={onChangeCategory(number)}
        className={number === category ? `bg-sky-200 text-left transition duration-500 shadow ease-in-out transform hover:-translate-y-1 hover:shadow-lg select-none cursor-pointer hover:bg-sky-200 click:active:checked:focus:bg-sky-200 dark:bg-gray-800 rounded-md flex flex-1 items-center p-4`
